...
export default function PostJob() {
  ...
  const [formData, setFormData] = useState({
    title: "", company: "", location: "", type: "CDI", sector: "", description: "", email: "",
  });

  const t = {
    fr: {
      badge: "Recruteurs", heading: "Publiez votre offre.", subheading: "Simple, rapide, visible par les bons candidats.",
      label_title: "Intitulé du poste", label_company: "Entreprise", label_location: "Lieu",
      label_type: "Type de contrat", label_sector: "Secteur", label_description: "Description du poste",
      label_email: "Email de contact", placeholder_title: "ex. Analyste KYC Senior",
      placeholder_company: "ex. Goldman Sachs", placeholder_location: "ex. Luxembourg, Paris, Remote",
      placeholder_description: "Décrivez le poste, les missions, le profil recherché...",
      placeholder_email: "recrutement@entreprise.com", btn_submit: "Publier l'offre",
      btn_loading: "Publication en cours...", success_title: "Offre publiée avec succès.",
    ...
    en: {
      badge: "Recruiters", heading: "Post your job.", subheading: "Simple, fast, visible to the right candidates.",
      label_title: "Job title", label_company: "Company", label_location: "Location",
      label_type: "Contract type", label_sector: "Sector", label_description: "Job description",
      label_email: "Contact email", placeholder_title: "e.g. Senior KYC Analyst",
      placeholder_company: "e.g. Goldman Sachs", placeholder_location: "e.g. Luxembourg, Paris, Remote",
      placeholder_description: "Describe the role, responsibilities, required profile...",
      placeholder_email: "recruitment@company.com", btn_submit: "Post the job",
      btn_loading: "Publishing...", success_title: "Job published successfully.",
  ...
  const handleChange = function(e) { setFormData(Object.assign({}, formData, { [e.target.name]: e.target.value })); };
  ...
  const handleSubmit = async function(e) {
    ...
    const { error } = await supabase.from("jobs").insert([{
      ...
      type: formData.type, sector: formData.sector, description: formData.description, email: formData.email,
    }]);
    setLoading(false);
    if (error) { setError("Une erreur est survenue. Veuillez réessayer."); }
    ...
    <div className="min-h-screen bg-white text-gray-900 flex flex-col" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-200 flex items-center justify-between px-4 md:px-8 h-16">
        ...
        <nav className="flex items-center gap-3 md:gap-6 ml-4">
          ...
          <Link href="/post-job" className="text-sm text-gray-900 font-semibold">Recruteurs</Link>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <select name="type" value={formData.type} onChange={handleChange}
                    ...
                    {copy.types.map(function(t) { return <option key={t}>{t}</option>; })}
                  </select>
                </div>
                <div>
                  ...
                  <select name="sector" value={formData.sector} onChange={handleChange}
                    className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-gray-900 transition bg-white">
                    <option value="">—</option>
                    {copy.sectors.map(function(s) { return <option key={s}>{s}</option>; })}
                  </select>
                </div>
              </div>
              <div>
