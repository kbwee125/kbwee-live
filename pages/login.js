...
export default function Login() {
  ...
  const handleGoogleLogin = async function() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: "https://kbwee.com/profil",
      },
    });
    if (error) {
      ...
      <main className="flex-grow flex items-center justify-center px-4 pt-16">
        <div className="w-full max-w-sm py-16">
          ...
          <div className="border border-gray-200 rounded-2xl p-8">
            ...
            {/* Bouton Google en premier — c'est le plus important */}
            <button
              onClick={handleGoogleLogin}
              disabled={loading}
              className="w-full border-2 border-gray-200 rounded-xl py-3 text-sm font-medium text-gray-700 hover:border-gray-900 hover:text-gray-900 transition flex items-center justify-center gap-3 mb-6 disabled:opacity-50">
              ...
              {loading ? copy.loadingGoogle : copy.googleBtn}
            </button>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <button type="submit" disabled={loading}
                className="w-full bg-gray-900 text-white py-3 rounded-xl text-sm font-medium hover:opacity-80 transition mt-2 disabled:opacity-50">
                {loading ? "..." : (isLogin ? copy.btnLogin : copy.btnSignup)}
              </button>
            </form>
          </div>
